...
class login:
    ...
    def show_dasbor_akun(self):
        ...
        print('%sâ•­â”€â”€â”€â”€â”€â”€â”€â”€â”€ [ %sData Akun %s] â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•®  â•­â”€â”€â”€â”€â”€â”€â”€â”€â”€ [ %sData User %s] â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•®%s'%(y,q,y,q,y,z))
        print('%sâ”‚%sâ”‚%sâ”‚%sâ”‚%s'%(y,' '*34,' '*2,' '*34,z))
        if len(x['Akun']) > 18: a_ = '%sâ”‚    %sAkun  : %s...    %sâ”‚'%(y,z,x['Akun'][:15],y)
        else: a_ = '%sâ”‚    %sAkun  : %s%s    %sâ”‚'%(y,z,x['Akun'],' '*(18-len(x['Akun'])),y)
        b_ = '%sâ”‚    %sID    : %s%s    %sâ”‚'%(y,z,x['ID'],' '*(18-len(x['ID'])),y)
        c_ = '%sâ”‚    %sBuat  : %s%s    %sâ”‚'%(y,z,x['Pembuatan'],' '*(18-len(x['Pembuatan'])),y)
        d_ = '%sâ”‚    %sTeman : %s%s    %sâ”‚'%(y,z,x['Teman'],' '*(18-len(x['Teman'])),y)
        e_ = '%sâ”‚    %sFolls : %s%s    %sâ”‚'%(y,z,x['Followers'],' '*(18-len(x['Followers'])),y)
        if len(x['User']) > 18: a__ = '%s  %sâ”‚    %sUser  : %s...    %sâ”‚'%(a_,y,z,x['User'][:15],y)
        else: a__ = '%s  %sâ”‚    %sUser  : %s%s    %sâ”‚'%(a_,y,z,x['User'], ' '*(18-len(x['User'])),y)
        b__ = '%s  %sâ”‚    %sPrem  : %s%s    %sâ”‚'%(b_,y,z, x['Premium'],' '*(18-len(x['Premium'])),y)
        c__ = '%s  %sâ”‚    %sAktif : %s%s    %sâ”‚'%(c_,y,z, x['Aktivasi'],' '*(18-len(x['Aktivasi'])),y)
        d__ = '%s  %sâ”‚    %sExprd : %s%s    %sâ”‚'%(d_,y,z, x['Kadaluwarsa'],' '*(18-len(x['Kadaluwarsa'])),y)
        e__ = '%s  %sâ”‚    %sIP    : %s%s    %sâ”‚'%(e_,y,z, x['IP'],' '*(18-len(x['IP'])),y)
        print('%s\n%s\n%s\n%s\n%s'%(a__,b__,c__,d__,e__))
        print('%sâ”‚%sâ”‚%sâ”‚%sâ”‚%s'%(y,' '*34,' '*2,' '*34,z))
        print('%sâ•°â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•¯  â•°â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•¯\n'%(y))
